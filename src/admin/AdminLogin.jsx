...
// Same Supabase auth as the tenant Login page (one account can be both a
// business member and a platform admin) — this just brands the screen
// distinctly and lands back on /admin instead of / after sign-in, so an
// admin never has to detour through the tenant app to get here.
export default function AdminLogin() {
  ...
  return (
    <div className="flex h-screen w-full items-center justify-center px-4" style={{ backgroundColor: C.navyDeep }}>
      <form onSubmit={handleSubmit} className="w-full max-w-sm rounded-2xl bg-white p-8 shadow-sm">
        <div className="mb-6 flex flex-col items-center text-center">
          <div className="mb-3 flex h-14 w-14 items-center justify-center rounded-2xl" style={{ backgroundColor: C.navy }}>
            <ShieldCheck size={26} color={C.white} />
          </div>
          <h1 className="text-lg font-bold" style={{ color: C.ink }}>{t("admin.login.title")}</h1>
          <p className="mt-1 text-xs" style={{ color: C.slateLight }}>{t("admin.login.subtitle")}</p>
        </div>
        ...
        <p className="mt-5 text-center text-xs" style={{ color: C.slateLight }}>
          <Link to="/" className="font-semibold" style={{ color: C.green }}>{t("admin.login.backToApp")}</Link>
        </p>
      </form>
    </div>
  );
